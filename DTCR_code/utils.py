...
def get_fake_sample(data):
    sample_nums, series_len, embedding_size = data.shape

    # Ensure mask matches the shape of `data`
...
def load_data(filename):
    ...
    # Reshape data to match RNN input expectations
    num_steps = data.shape[1]
    data = data.reshape((-1, num_steps, 1))  # Assuming embedding_size=1
...
def next_batch(batch_size, data):
    ...
    if left_row != 0:
        need_more = batch_size - left_row
        need_more = np.random.choice(np.arange(row), size=need_more)
        batch_input = np.concatenate((data[-left_row:, :, :], data[need_more]), axis=0)
        yield (batch_input, True)

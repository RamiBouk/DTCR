...
class RNN_clustering_model(object):
    ...
    def build_model(self):
        ...
        input = tf.compat.v1.placeholder(tf.float32, [None, self.num_steps], name='inputs')  # input
        noise = tf.compat.v1.placeholder(tf.float32, [None, self.num_steps], name='noise')

        real_fake_label = tf.compat.v1.placeholder(tf.float32, [None, 2], name='real_fake_label')

        F_new_value = tf.compat.v1.placeholder(tf.float32, [None, self.K], name='F_new_value')
...
def run_model(train_data_filename, config):
    ...
    for i in range(Epoch):
        ...
        noise_data= np.random.normal(loc=0, 
                                     scale=0.1, 
                                     size=[shuffle_data.shape[0]*2, shuffle_data.shape[1]])
        total_abstract = []
        print(('----------Epoch %d----------' % i))
        k = 0
        ...
        for input, _ in utils.next_batch(config.batch_size, shuffle_data):
            ...
            with tf.GradientTape() as tape:
                loss_val, abstract, _ = model(input, fake_input, noise, train_real_fake_labels)
...
#            noise = noise_data[k * config.batch_size * 2: (k + 1) * config.batch_size * 2, :]
#        fake_input, train_real_fake_labels = utils.get_fake_sample(input)
#        loss_val, abstract, _ = sess.run( [loss_tensors['loss'], real_hidden_abstract, train_op],
